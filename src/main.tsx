    ...
    <div className="flex min-h-dvh items-center justify-center bg-menu-bg px-6 text-center [font-family:'Sen',Helvetica]">
      <div className="max-w-sm rounded-2xl bg-white p-6 shadow-menu-md">
        ...
        <p className="mt-2 text-sm text-menu-text-soft">
          Essa rota existe só para o preview não quebrar. O foco do editor é a
          tela MenuPage.
        </p>
        {params.id && (
          <p className="mt-2 text-xs text-menu-text-soft">ID: {params.id}</p>
        )}
        <Link
          to="/"
  ...
  return (
    <BrowserRouter>
      <Routes>
        ...
        <Route path="/r/:slug/produto/:id" element={<ProductPage />} />
        <Route
          path="/carrinho"
          element={<PlaceholderPage title="Carrinho" />}
        />
        <Route
          path="/meus-pedidos"
          element={<PlaceholderPage title="Meus pedidos" />}
        />
