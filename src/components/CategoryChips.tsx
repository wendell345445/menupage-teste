...
export function CategoryChips({
  ...
  const hasAllCategory = useMemo(
    () => categories.some((category) => category.id === null),
    [categories],
  );
    ...
    <>
      ...
      >
        <div className="relative -mx-4 overflow-hidden bg-transparent transition-colors duration-200 sm:-mx-6 md:-mx-8">
          {!isLoading && hasAllCategory ? (
            <div className="pointer-events-none absolute inset-y-0 left-0 z-30 flex w-[38px] items-center bg-white pl-2 sm:w-[44px] sm:pl-3 md:w-[50px] md:pl-4">
              <button
                ref={menuButtonRef}
                type="button"
                ...
                className="pointer-events-auto relative z-10 flex h-8 w-6 shrink-0 items-center justify-center rounded-full text-[#5c5555] transition-colors hover:bg-black/[0.04] active:bg-black/[0.07]"
              >
                <svg
                  aria-hidden="true"
          ...
          <nav
            ...
            className="relative z-20 flex w-full touch-pan-x items-center gap-2.5 overflow-x-auto overflow-y-hidden py-1.5 pl-[38px] pr-4 overscroll-x-contain [-ms-overflow-style:none] [scrollbar-width:none] sm:pl-[44px] sm:pr-6 md:pl-[50px] md:pr-8 [&::-webkit-scrollbar]:hidden"
          >
            {isLoading
